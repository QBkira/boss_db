...
table_exists(Pid, Type) when is_atom(Type) ->
  TableName = boss_record_lib:database_table(Type),
  table_exists(Pid, TableName);
table_exists(Pid, TableName) when is_list(TableName) ->
...
create_table(Pid, Type, TableDefinition) when is_atom(Type) ->
  TableName = boss_record_lib:database_table(Type),
  create_table(Pid, TableName, TableDefinition);
create_table(Pid, TableName, TableDefinition) when is_list(TableName) ->
  Res = fetch(Pid, ["CREATE TABLE ", TableName, " ( ", tabledefinition_to_sql(TableDefinition), " )"]),
  case Res of
    {updated, _} ->
      ok;
    {error, MysqlRes} ->
...
get_migrations_table(Pid) ->
  Res = fetch(Pid, "SELECT * FROM schema_migrations", []),
  case Res of
    {data, {mysql_result, _Colums, ResultRows, _, _, _}} ->
      ResultRows;
    {error, Reason} ->
      {error, Reason}
  end.
...
migration_done(Pid, Tag, up) ->
  Res = fetch(Pid, "INSERT INTO schema_migrations (version, migrated_at) values ($1, NOW())",
    [atom_to_list(Tag)]),
  case Res of
    {updated, _Result} ->
      lager:info("migration ~p up done", [Tag]),
      ok;
    {error, Reason} ->
      lager:error("schema_migrations error: ~p", [Reason]),
      {error, Reason}
  end;
migration_done(Pid, Tag, down) ->
  Res = fetch(Pid, "DELETE FROM schema_migrations WHERE version = $1", [atom_to_list(Tag)]),
  case Res of
    {updated, _Result} ->
      lager:info("migration ~p down done", [Tag]),
      ok;
    {error, Reason} ->
      lager:error("schema_migrations error: ~p", [Reason]),
      {error, Reason}
...
lookup_single_parameter(Position, Parameters) ->
  try lists:nth(Position, Parameters) of
    ...
    Error ->
      throw(io_lib:format("Error (~p) getting parameter $~w. Provided Params: ~p", [Error, Position, Parameters]))
  end.
